...
Drag a folder onto the workspace list in the sidebar. Claudette opens it as a workspace named after the folder and uses that directory as the working directory. It does **not** create a new git worktree or branch.

The folder has to be inside a git checkout (the repository root, a subdirectory, or an existing linked worktree). If that repository is not in Claudette yet, it is added automatically. Dropping the same folder again selects the workspace that already points at it. A detached HEAD is refused — check out a branch first. A file dropped on the list is rejected. Drops elsewhere in the window still attach to the chat.
